...
const {
  buildFilepathLookup,
} = require("./webpack-utils/build-filepath-lookup");
...
module.exports = () => {
  return new Promise((resolve, _reject) => {
    const createWebpackConfig = function () {
      ...
      if (APP_ROOT.includes(ROOT_DIR)) {
        // should only return truthy for running Arches-core without a project
      ...
      const archesCoreEntryPointConfiguration = buildFilepathLookup(
        Path.resolve(__dirname, ROOT_DIR, "app", "media", "js"),
      );
      const projectEntryPointConfiguration = buildFilepathLookup(
        Path.resolve(__dirname, APP_ROOT, "media", "js"),
      );

      const archesApplicationsEntrypointConfiguration =
      ...
      const archesCoreJavascriptRelativeFilepathToAbsoluteFilepathLookup =
        Object.entries(archesCoreEntryPointConfiguration).reduce(
          (acc, [path, config]) => {
      ...
      const archesApplicationsJavascriptRelativeFilepathToAbsoluteFilepathLookup =
        Object.entries(archesApplicationsEntrypointConfiguration).reduce(
          (acc, [path, config]) => {
            acc[path + "$"] = Path.resolve(__dirname, path, config["import"]);
            return acc;
          },
      ...
      if (!fs.existsSync(archesCorePackageJSONFilepath)) {
        archesCorePackageJSONFilepath = Path.resolve(
          __dirname,
          PROJECT_RELATIVE_NODE_MODULES_PATH,
          "arches",
          "package.json",
        );
      }
      ...
      parsedPackageJSONFilepaths[
        Path.join(archesCorePackageJSON.name, "package.json").replace(
          /\\/g,
          "/",
        )
      ] = archesCorePackageJSONFilepath;

      const parsedArchesCoreNodeModulesAliases = Object.entries(
        archesCorePackageJSON["nodeModulesPaths"],
      ).reduce((acc, [alias, subPath]) => {
        if (subPath.slice(0, 7) === "plugins") {
          // handles for node_modules -esque plugins in arches core
          acc[alias] = Path.resolve(
            __dirname,
            ROOT_DIR,
            "app",
            "media",
      ...
      const projectJSONFilepath = Path.resolve(
        __dirname,
        PROJECT_RELATIVE_NODE_MODULES_PATH,
        "package.json",
      );
      if (fs.existsSync(projectJSONFilepath)) {
        // handles running Arches without a project
        projectPackageJSON = require(projectJSONFilepath);
        parsedPackageJSONFilepaths[
          Path.join(projectPackageJSON.name, "package.json").replace(/\\/g, "/")
        ] = projectJSONFilepath;

        parsedProjectNodeModulesAliases = Object.entries(
      ...
      for (const archesApplication of ARCHES_APPLICATIONS) {
        try {
          ...
          if (
            !ARCHES_APPLICATIONS_PATHS[archesApplication].includes(
              "site-packages",
            )
          ) {
            // if the path doesn't include site-packages then we can assume it's linked via egg/wheel
            archesApplicationJSONFilepath = Path.resolve(
              __dirname,
              ARCHES_APPLICATIONS_PATHS[archesApplication],
              "..",
              "package.json",
          ...
          const archesApplicationPackageJSON = require(
            archesApplicationJSONFilepath,
          );
          parsedPackageJSONFilepaths[
            Path.join(
              archesApplicationPackageJSON.name,
              "package.json",
            ).replace(/\\/g, "/")
          ] = archesApplicationJSONFilepath;

          for (const [alias, subPath] of Object.entries(
            archesApplicationPackageJSON["nodeModulesPaths"],
          )) {
            if (
              ...
              parsedArchesApplicationsNodeModulesAliases[alias] = Path.resolve(
                __dirname,
                PROJECT_RELATIVE_NODE_MODULES_PATH,
                "..",
                subPath,
      ...
      const coreArchesTemplatePathConfiguration = buildFilepathLookup(
        Path.resolve(__dirname, ROOT_DIR, "app", "templates"),
      );
      const projectTemplatePathConfiguration = buildFilepathLookup(
      ...
      const coreArchesImagePathConfiguration = buildFilepathLookup(
        Path.resolve(__dirname, ROOT_DIR, "app", "media", "img"),
        STATIC_URL,
      );
      const projectImagePathConfiguration = buildFilepathLookup(
        Path.resolve(__dirname, APP_ROOT, "media", "img"),
        STATIC_URL,
      );

      const archesApplicationsImagePathConfiguration =
        ARCHES_APPLICATIONS.reduce((acc, archesApplication) => {
          return {
            ...acc,
            ...buildFilepathLookup(
      ...
      const coreArchesCSSFilepathConfiguration = buildFilepathLookup(
        Path.resolve(__dirname, ROOT_DIR, "app", "media", "css"),
      );
      const projectCSSFilepathConfiguration = buildFilepathLookup(
        Path.resolve(__dirname, APP_ROOT, "media", "css"),
      );

      const archesApplicationsCSSFilepaths = [];
      const archesApplicationsCSSFilepathConfiguration =
        ARCHES_APPLICATIONS.reduce((acc, archesApplication) => {
      ...
      const archesApplicationsVuePaths = ARCHES_APPLICATIONS.reduce(
      ...
      const universalConstants = {
        ...
        SITE_PACKAGES_DIRECTORY: JSON.stringify(
      ...
      for (const archesApplication of ARCHES_APPLICATIONS) {
        if (
          ...
          universalConstants[
            `LINKED_APPLICATION_PATH_${linkedApplicationPathCount}`
          ] = JSON.stringify(
            ARCHES_APPLICATIONS_PATHS[archesApplication],
          ).replace(/\\/g, "/");
            ...
            },
            {
              ...
              options: {
                ...
                preprocessor: async (content, loaderContext) => {
                  ...
                  if (resourcePath.includes(APP_ROOT)) {
                    // project-level component
                    templatePath = resourcePath.split(APP_ROOT)[1];
                  }

                  for (const archesApplicationPath of Object.values(
                    ARCHES_APPLICATIONS_PATHS,
                  )) {
                    // arches application component
                    if (
                      !templatePath &&
                      resourcePath.includes(archesApplicationPath)
                    ) {
                      templatePath = resourcePath.split(
                  ...
                  console.log(
                    `Loading "${templatePath}" from Django server...`,
                  );
                    ...
                    if (failureCount < 5) {
                      ...
                      if (!isTestEnvironment) {
                        loaderContext.emitError(
                          `Unable to fetch ${templatePath} from the Django server.`,
                        );
                      } else {
                        console.warn(
    ...
    const runPythonScript = (pythonCommand) => {
      ...
      projectSettings.stdout.on("data", function (data) {
        ...
        global.WEBPACK_DEVELOPMENT_SERVER_PORT =
          parsedData["WEBPACK_DEVELOPMENT_SERVER_PORT"];
